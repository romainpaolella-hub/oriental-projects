...
export const villaDispo = defineType({
  ...
  fields: [
    ...
    defineField({name: 'brochureFr', title: 'Brochure — Français (PDF)', type: 'file', options: {accept: 'application/pdf'}, hidden: ({parent}) => parent?.isPlaceholder}),
    defineField({name: 'brochureEn', title: 'Brochure — English (PDF)', type: 'file', options: {accept: 'application/pdf'}, hidden: ({parent}) => parent?.isPlaceholder}),
    defineField({
      name: 'linkHref',
